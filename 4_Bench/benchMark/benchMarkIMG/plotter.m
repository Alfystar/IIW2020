...
analizeDatFile(path,"png");
analizeDatFile(path,"pdf");

out=0;
end

function sfile = analizeDatFile(file,format)

datValue = retrieveData(file);

    dot = strfind(file, '.');
    folder = extractBefore(file, dot(end))+"_" + format + "/"; %% use last '.' to get path
...
for con = 1:length(nParCon)
    ...
    fprintf('\t');
    disp(path + "." + format)
    exportgraphics(resp_fig, path + "." + format,'ContentType','vector')
end
...
nMaxCore = max(subDat.(1)); % max number of core
...
%     subDat = subDat(1:indexes(1)-1, :); % we reduce size of subDat for the next iteration of find
    subDat(indexes,:)=[]; % we reduce size of subDat for the next iteration of find
    plot(valueTab(:,1), valueTab(:,2), '-*', 'DisplayName', displayName);
    grid on
    hold on
    
    if(size(subDat,1) == 0) % we reached the minimum amount of core
        break;
    end
