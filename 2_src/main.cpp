...
void help() {
    ...
    cout << "\t 1) >> badAlphaWeb                          := Home server al PWD corrente\n";
    cout << "\t 2) >> badAlphaWeb <webServer home Path>    := Home server spostata alla dir desiderata\n";
    ...
    if (errorRet != 0)    //un qualche errore nel ragiungimento della cartella
    {
        switch (errno) {
            case 2: //No such file or directory
                printf("directory non exists, now proceed to creation\n");
...
int main(int argc, char *argv[]) {
    if (argc > 1) {
        if (strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "--help") == 0) {
            help();
            exit(-1);
        } else if (StartServerStorage(argv[1])) {
            cout << "main Direcotry change fail\n";
            exit(-1);
        }
    } else {
        cout << "Directory change success\n";
